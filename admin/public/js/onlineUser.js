  ...
  window.parent.client.request('onlineUser', null, function(err, msg) {
    ...
    for (var sid in msg) {
		...
		// }
			...
			totalConnCount += server.loginedCount;
			loginedCount += server.loginedCount;

      var users = {};
      var lists = server.loginedList;
      if (lists)
        for (var i = 0; i < lists.length; i++) {
          var username = lists[i].uid;
          list.push({
            address: lists[i].address,
            serverId: sid,
            username: username,
            loginTime: new Date(lists[i].loginTime).Format(
              'yyyy-MM-dd hh:mm:ss'
            ),
            uid: lists[i].uid
          });
